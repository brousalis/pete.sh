...
export interface TrainingImpulse {
  ...
  intensity: 'recovery' | 'easy' | 'moderate' | 'tempo' | 'threshold' | 'vo2max' | 'anaerobic'
...
export interface HrRecovery {
  ...
  recoveryRating: 'excellent' | 'good' | 'average' | 'below_average' | 'poor' | 'unknown'
  recommendation: string
}
...
export function calculateTRIMP(
  ...
  const trimp = durationMinutes * clampedReserve * Math.exp(coefficient * clampedReserve)
  ...
  if (hrPercentMax < 0.60) {
    intensity = 'recovery'
    recommendation = 'Great recovery effort. This intensity promotes active recovery without adding training stress.'
  } else if (hrPercentMax < 0.70) {
    intensity = 'easy'
    recommendation = 'Good easy/base building pace. This builds aerobic foundation efficiently.'
  } else if (hrPercentMax < 0.80) {
    intensity = 'moderate'
    recommendation = 'Moderate effort. Good for building aerobic capacity and running economy.'
  } else if (hrPercentMax < 0.85) {
    intensity = 'tempo'
    recommendation = 'Tempo effort. This improves lactate threshold and sustained pace ability.'
  } else if (hrPercentMax < 0.90) {
    intensity = 'threshold'
    recommendation = 'Threshold work. High training stimulus - ensure adequate recovery before next hard session.'
  } else if (hrPercentMax < 0.95) {
    intensity = 'vo2max'
    recommendation = 'VO2max effort. Maximum aerobic stimulus - take at least 48 hours before another intense session.'
  } else {
    intensity = 'anaerobic'
    recommendation = 'Anaerobic/max effort. Full recovery required - 48-72 hours before next quality session.'
  }
 ...
 */
export function analyzeCardiacDrift(hrSamples: HrSample[]): CardiacDriftAnalysis {
  ...
  const driftPercentage = ((secondHalfAvgHr - firstHalfAvgHr) / firstHalfAvgHr) * 100
  ...
  if (driftPercentage < 3) {
    interpretation = 'minimal'
    recommendation = 'Excellent cardiac efficiency. Your aerobic system handled this effort well with minimal heart rate drift.'
  } else if (driftPercentage < 5) {
    interpretation = 'moderate'
    recommendation = 'Normal cardiac drift. Consider hydrating more during longer runs or on warmer days.'
  } else if (driftPercentage < 8) {
    interpretation = 'significant'
    recommendation = 'Notable cardiac drift. This may indicate dehydration, heat stress, or pushing beyond current aerobic capacity. Focus on hydration and building aerobic base.'
  } else {
    interpretation = 'excessive'
    recommendation = 'High cardiac drift detected. Review hydration, pacing strategy, and consider more easy aerobic runs to build base fitness.'
  }
...
export function analyzeAerobicDecoupling(
  ...
  const firstHalfPace = average(paceSamples.slice(0, paceMidpoint).map(s => s.minutes_per_mile))
  const secondHalfPace = average(paceSamples.slice(paceMidpoint).map(s => s.minutes_per_mile))
  ...
  const decouplingPercentage = ((firstHalfEfficiency - secondHalfEfficiency) / firstHalfEfficiency) * 100
  ...
  if (decouplingPercentage < 3.5) {
    ...
    recommendation = 'Outstanding aerobic efficiency. Your pace:HR relationship held steady throughout - you\'re well-prepared for endurance events at this effort level.'
  } else if (decouplingPercentage < 5) {
    interpretation = 'good'
    recommendation = 'Good aerobic coupling. Minor efficiency loss is normal. Continue building your aerobic base with easy miles.'
  } else if (decouplingPercentage < 8) {
    interpretation = 'moderate'
    recommendation = 'Moderate decoupling detected. For races at this duration, consider starting more conservatively or focus on more aerobic base work.'
  } else {
    interpretation = 'poor'
    recommendation = 'Significant decoupling. Either this effort was too hard for current fitness, or environmental factors (heat, dehydration) affected performance. Build more aerobic base before racing at this intensity.'
  }
 ...
 */
export function analyzeCadence(cadenceSamples: CadenceSample[]): CadenceAnalysis {
  ...
  let recommendation: string
  if (avg < 160) {
    recommendation = 'Cadence is low. Try shortening your stride and increasing turnover - aim for 170-180 SPM. This reduces impact forces and improves running economy.'
  } else if (avg < 170) {
    recommendation = 'Cadence is slightly below optimal. Small increases (5-10 SPM) can improve efficiency. Use a metronome app during some runs.'
  } else if (avg <= 185) {
    recommendation = 'Excellent cadence! You\'re in the optimal range that elite runners target. This reduces injury risk and maximizes efficiency.'
  } else {
    recommendation = 'Cadence is high. While not necessarily bad, ensure you\'re not sacrificing stride length. Focus on smooth, powerful strides.'
  }
  
  if (consistency === 'highly_variable' || consistency === 'variable') {
    recommendation += ' Work on maintaining consistent cadence throughout your run, especially when fatigued.'
  }
...
export function analyzePace(
  ...
  const values = paceSamples.map(s => s.minutes_per_mile).filter(v => v > 0 && v < 30)
  ...
  if (splits.length >= 2) {
    ...
    if (paceVariation > 0.15) {
      ...
      recommendation = 'Highly variable pacing detected. For optimal performance, aim for more consistent effort throughout your run.'
    } else if (paceDiff < -0.15) { // Second half faster by more than 9 sec/mile
      splitStrategy = 'negative'
      recommendation = 'Great negative split! Running the second half faster is the optimal racing strategy and shows good pacing discipline.'
    } else if (paceDiff > 0.25) { // Second half slower by more than 15 sec/mile
      splitStrategy = 'positive'
      recommendation = 'Positive split detected (slowed down). Consider starting more conservatively to maintain pace throughout. This is common but leaves time on the table.'
    } else {
      splitStrategy = 'even'
      recommendation = 'Even pacing - this is excellent for training runs and shows good effort management.'
    }
  } else {
...
export function calculateSplits(
  ...
  const avgPaceOverall = paceSamples.length > 0 
    ? average(paceSamples.map(s => s.minutes_per_mile))
    : totalDurationSeconds / 60 / totalDistanceMiles
  
  const avgHrOverall = hrSamples.length > 0 
    ? average(hrSamples.map(s => s.bpm))
    : 0
  ...
  if (numFullMiles > 0 && paceSamples.length > 0) {
    ...
    for (let i = 0; i < numFullMiles; i++) {
      ...
      if (cadenceSamples.length > 0) {
        const cadenceStartIdx = Math.floor(i * cadenceSamples.length / numFullMiles)
        const cadenceEndIdx = Math.floor((i + 1) * cadenceSamples.length / numFullMiles)
        const splitCadence = cadenceSamples.slice(cadenceStartIdx, cadenceEndIdx)
        if (splitCadence.length > 0) {
          avgCadence = Math.round(average(splitCadence.map(s => s.steps_per_minute)))
        }
      }
      ...
      splits.push({
        ...
        paceVsAvg: Math.round(((avgPace - avgPaceOverall) / avgPaceOverall) * 100 * 10) / 10,
        hrVsAvg: avgHrOverall > 0 ? Math.round(((avgHr - avgHrOverall) / avgHrOverall) * 100 * 10) / 10 : 0,
...
export function generateTimeSeriesData(
  ...
  // Convert samples to normalized format with elapsed seconds
  ...
  hrSamples.forEach(sample => {
    const elapsed = Math.round((parseTimestamp(sample.timestamp).getTime() - startTime.getTime()) / 1000)
    hrMap.set(elapsed, sample.bpm)
  })
  ...
  for (let elapsed = 0; elapsed <= durationSeconds; elapsed += interval) {
    ...
    for (let offset = 0; offset <= 30; offset++) {
      if (hr === null && hrMap.has(elapsed + offset)) hr = hrMap.get(elapsed + offset)!
      if (hr === null && hrMap.has(elapsed - offset)) hr = hrMap.get(elapsed - offset)!
      if (cadence === null && cadenceMap.has(elapsed + offset)) cadence = cadenceMap.get(elapsed + offset)!
      if (cadence === null && cadenceMap.has(elapsed - offset)) cadence = cadenceMap.get(elapsed - offset)!
      if (pace === null && paceMap.has(elapsed + offset)) pace = paceMap.get(elapsed + offset)!
      if (pace === null && paceMap.has(elapsed - offset)) pace = paceMap.get(elapsed - offset)!
    }
...
export function generateInsights(
  ...
  if (cardiacDrift.interpretation === 'minimal') {
    insights.push({
      ...
      description: 'Your heart rate stayed stable throughout the workout, indicating good hydration and aerobic fitness.',
  ...
  if (aerobicDecoupling.interpretation === 'excellent') {
    insights.push({
      ...
      description: 'Your pace:HR relationship held steady - excellent endurance indicator.',
  ...
  if (cadenceAnalysis.optimalRange) {
    insights.push({
      ...
      description: 'Your cadence is in the 170-180 SPM sweet spot that elite runners target.',
    ...
    insights.push({
      ...
      description: 'Increasing your cadence toward 170-180 SPM can reduce injury risk and improve efficiency.',
      metric: `${cadenceAnalysis.average} SPM`,
      icon: 'footprints'
    })
  }
  ...
  if (paceAnalysis.splitStrategy === 'negative') {
    insights.push({
      ...
      description: 'Running the second half faster is the gold standard for pacing!',
      icon: 'award'
    })
  } else if (paceAnalysis.splitStrategy === 'positive' && durationMinutes > 20) {
    insights.push({
      category: 'improvement',
      title: 'Pacing Strategy',
      description: 'You slowed in the second half. Try starting 5-10 seconds/mile slower to finish stronger.',
      icon: 'clock'
    })
  }
  ...
  if (peakZone && peakZone.percentage > 50) {
    insights.push({
      ...
      description: 'Over half your workout was in the peak HR zone. Ensure adequate recovery before your next hard session.',
    ...
    insights.push({
      ...
      description: 'Great zone 2/3 work! This effort builds aerobic capacity and fat-burning efficiency.',
...
export function computeEnhancedAnalytics(
  ...
  // Basic stats
  const avgHr = workout.hr_average || (hrSamples.length > 0 ? Math.round(average(hrSamples.map(s => s.bpm))) : 0)
  const avgCadence = workout.cadence_average || (cadenceSamples.length > 0 ? Math.round(average(cadenceSamples.map(s => s.steps_per_minute))) : null)
  const avgPace = workout.pace_average || (paceSamples.length > 0 ? average(paceSamples.map(s => s.minutes_per_mile)) : (distanceMiles > 0 ? durationMinutes / distanceMiles : 0))
  ...
  const hrVariability = {
    ...
    range: 0,
    avgFirstQuarter: quarterLength > 0 ? Math.round(average(hrValues.slice(0, quarterLength))) : avgHr,
    avgLastQuarter: quarterLength > 0 ? Math.round(average(hrValues.slice(-quarterLength))) : avgHr
  }
  hrVariability.range = hrVariability.max - hrVariability.min
  
  // Advanced analytics
  const cardiacDrift = analyzeCardiacDrift(hrSamples)
  const aerobicDecoupling = analyzeAerobicDecoupling(hrSamples, paceSamples, durationSeconds)
  const cadenceAnalysis = analyzeCadence(cadenceSamples)
  const paceAnalysis = analyzePace(paceSamples, [])
  const trainingImpulse = calculateTRIMP(durationMinutes, avgHr, restingHr, maxHr)
  
  // Calculate efficiency factor
  const efficiencyFactor = avgHr > 0 && avgPace > 0 ? Math.round((1 / avgPace) * 100 / avgHr * 1000) / 1000 : 0
  
  // Calculate splits
  const splits = calculateSplits(hrSamples, paceSamples, cadenceSamples, distanceMiles, durationSeconds)
  ...
  const hrRecovery: HrRecovery = {
    ...
    recommendation: 'HR recovery data not available. Consider tracking your heart rate for 2 minutes after workouts.'
  }
  
  // Generate time series for charts
  const timeSeriesData = generateTimeSeriesData(hrSamples, cadenceSamples, paceSamples, startTime, durationSeconds)
