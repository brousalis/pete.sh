...
End-to-end brief for how PeteCoach was planned and how it actually works. Read this before changing
coaching behaviour. Ops leftovers (worker, deploy, do-not-rebuild iOS) live in
[`PETECOACH-NEXT-STEPS.md`](./PETECOACH-NEXT-STEPS.md).

PeteCoach is not a chatbot bolted onto fitness. It is a periodized plan with a deterministic injury
...
- **Devices:** Apple Watch S12 only. Garmin retired except a one-time historical export. Coospo
  sensors pair to the watch. PeteTrain iOS is the HealthKit pipe. Strava API is not used (2026 AI
  ban).
...
PM2 name `petecoach-worker`, port 3021, cwd `apps/coach-worker`, env from `apps/web/.env`. Timezone
