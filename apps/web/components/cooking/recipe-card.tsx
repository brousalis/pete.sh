'use client'

import { Badge } from '@/components/ui/badge'
import { Card, CardContent } from '@/components/ui/card'
import { Skeleton } from '@/components/ui/skeleton'
import { transitions } from '@/lib/animations'
import type { Recipe } from '@/lib/types/cooking.types'
import { cn, resolveRecipeImageUrl } from '@/lib/utils'
import { motion } from 'framer-motion'
import { CalendarCheck, ChefHat, Clock, ExternalLink, Flame, Star, Users } from 'lucide-react'
import { AddToMealPlanPopover } from './add-to-meal-plan-popover'

interface RecipeCardProps {
  recipe: Recipe
  onClick?: () => void
  onToggleFavorite?: (recipe: Recipe) => void
  className?: string
  fridgeScore?: number
  fridgeFilterActive?: boolean
  cookCount?: number
  lastCookedAt?: string
  imageSize?: number
}

const difficultyColors = {
  easy: 'bg-accent-sage/15 text-accent-sage border-accent-sage/20',
  medium: 'bg-accent-gold/15 text-accent-gold border-accent-gold/20',
  hard: 'bg-accent-rose/15 text-accent-rose border-accent-rose/20',
}

function NutritionRow({ recipe }: { recipe: Recipe }) {
  const cal = recipe.calories_per_serving
  const protein = recipe.protein_g
  const carbs = recipe.carbs_g
  const fat = recipe.fat_g

  if (!cal && !protein && !carbs && !fat) return null

  return (
    <div className="flex items-center gap-2 text-[11px] tabular-nums">
      {cal != null && (
        <span className="flex items-center gap-0.5 text-accent-ember">
          <Flame className="size-3" />
          {Math.round(cal)}
        </span>
      )}
      {protein != null && (
        <span className="text-accent-azure">
          P {Math.round(protein)}g
        </span>
      )}
      {carbs != null && (
        <span className="text-accent-gold">
          C {Math.round(carbs)}g
        </span>
      )}
      {fat != null && (
        <span className="text-accent-rose">
          F {Math.round(fat)}g
        </span>
      )}
    </div>
  )
}

// ── Small recipe card (for sidebar grids) ──

export function RecipeCard({
  recipe,
  onClick,
  className,
  fridgeScore,
  fridgeFilterActive: isFridgeActive,
  cookCount,
}: RecipeCardProps) {
  const totalTime = (recipe.prep_time || 0) + (recipe.cook_time || 0)
  const resolvedImage = resolveRecipeImageUrl(recipe.image_url)

  return (
    <motion.div
      whileHover={{ y: -3 }}
      transition={transitions.springGentle}
    >
      <Card
        className={cn(
          'group cursor-pointer overflow-hidden border-border/50 transition-all hover:shadow-lg hover:border-border hover:bg-card/80',
          className
        )}
        onClick={onClick}
      >
        <CardContent className="p-0">
          <div className="relative aspect-[4/3] w-full overflow-hidden">
            {resolvedImage ? (
              <img
                src={resolvedImage}
                alt={recipe.name}
                className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
              />
            ) : (
              <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-muted/80 to-muted/40">
                <ChefHat className="size-6 text-muted-foreground/30" />
              </div>
            )}

            <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent" />

            <div className="absolute right-1.5 top-1.5 flex items-center gap-1">
              {recipe.source === 'trader_joes' && (
                <Badge className="bg-accent-rose text-white text-[9px] h-4 px-1 border-0 shadow-md">
                  TJ&apos;s
                </Badge>
              )}
              {isFridgeActive && fridgeScore != null && fridgeScore > 0 && (
                <FridgeMatchBadge score={fridgeScore} />
              )}
              {recipe.is_favorite && (
                <div className="rounded-full bg-black/40 p-0.5 backdrop-blur-sm">
                  <Star className="size-2.5 fill-accent-gold text-accent-gold" />
                </div>
              )}
            </div>

            <div className="absolute bottom-0 left-0 right-0 flex items-end justify-between px-2 pb-1.5">
              <div className="flex items-center gap-1">
                {totalTime > 0 && (
                  <Badge className="bg-black/40 text-white backdrop-blur-sm border-0 text-[9px] h-4 px-1 shadow-sm">
                    <Clock className="size-2 mr-0.5" />
                    {totalTime}m
                  </Badge>
                )}
              </div>
              <div
                className="opacity-0 transition-opacity group-hover:opacity-100"
                onClick={(e) => e.stopPropagation()}
              >
                <AddToMealPlanPopover
                  recipeId={recipe.id}
                  recipeName={recipe.name}
                  variant="icon"
                />
              </div>
            </div>
          </div>

          <div className="space-y-0.5 px-2.5 py-2">
            <div className="flex items-start justify-between gap-1.5">
              <h3 className="text-xs font-semibold leading-tight line-clamp-1 flex-1">
                {recipe.name}
              </h3>
              {recipe.source === 'trader_joes' && recipe.source_url && (
                <a
                  href={recipe.source_url}
                  target="_blank"
                  rel="noopener noreferrer"
                  onClick={(e) => e.stopPropagation()}
                  className="shrink-0 text-muted-foreground hover:text-foreground transition-colors"
                >
                  <ExternalLink className="size-2.5" />
                </a>
              )}
            </div>

            <div className="flex items-center gap-1 flex-wrap">
              {recipe.servings && (
                <span className="flex items-center gap-0.5 text-[10px] text-muted-foreground">
                  <Users className="size-2" />
                  {recipe.servings}
                </span>
              )}
              {recipe.difficulty && (
                <Badge
                  variant="outline"
                  className={cn(
                    'text-[9px] h-3.5 px-1 font-medium',
                    difficultyColors[recipe.difficulty]
                  )}
                >
                  {recipe.difficulty}
                </Badge>
              )}
              {cookCount != null && cookCount > 0 && (
                <span className="flex items-center gap-0.5 text-[10px] text-accent-sage/80">
                  <CalendarCheck className="size-2" />
                  {cookCount}×
                </span>
              )}
              {recipe.tags && recipe.tags.length > 0 && (
                <>
                  {recipe.tags.slice(0, 1).map((tag) => (
                    <Badge
                      key={tag}
                      variant="secondary"
                      className="text-[9px] h-3.5 px-1 font-normal"
                    >
                      {tag}
                    </Badge>
                  ))}
                  {recipe.tags.length > 1 && (
                    <span className="text-[9px] text-muted-foreground">
                      +{recipe.tags.length - 1}
                    </span>
                  )}
                </>
              )}
            </div>
          </div>
        </CardContent>
      </Card>
    </motion.div>
  )
}

function FridgeMatchBadge({ score }: { score: number }) {
  const pct = Math.round(score * 100)
  const color = pct >= 80 ? 'text-accent-sage' : pct >= 50 ? 'text-accent-gold' : 'text-muted-foreground'
  const bgColor = pct >= 80 ? 'bg-accent-sage/80' : pct >= 50 ? 'bg-accent-gold/80' : 'bg-black/50'

  return (
    <div className={cn('flex items-center gap-1 rounded-full px-1.5 py-0.5 backdrop-blur-sm shadow-sm', bgColor)}>
      <svg className="size-3.5" viewBox="0 0 16 16">
        <circle cx="8" cy="8" r="6" fill="none" stroke="currentColor" strokeWidth="2" className="text-white/20" />
        <circle
          cx="8"
          cy="8"
          r="6"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
          strokeDasharray={`${score * 37.7} 37.7`}
          strokeDashoffset="0"
          transform="rotate(-90 8 8)"
          className={color}
          strokeLinecap="round"
        />
      </svg>
      <span className={cn('text-[10px] font-semibold', color)}>
        {pct}%
      </span>
    </div>
  )
}

// ── Compact recipe card (horizontal layout for main grid) ──

export function RecipeCardCompact({
  recipe,
  onClick,
  className,
  fridgeScore,
  fridgeFilterActive: isFridgeActive,
  cookCount,
  imageSize = 140,
}: RecipeCardProps) {
  const totalTime = (recipe.prep_time || 0) + (recipe.cook_time || 0)
  const resolvedImage = resolveRecipeImageUrl(recipe.image_url)
  const hasNutrition = recipe.calories_per_serving || recipe.protein_g

  return (
    <div
      className={cn(
        'group flex flex-row rounded-lg border border-border/40 bg-card/50 cursor-pointer transition-all hover:bg-card hover:border-border hover:shadow-md active:scale-[0.98] overflow-hidden',
        className
      )}
      style={{ height: imageSize }}
      onClick={onClick}
    >
      {/* Left image */}
      <div className="relative shrink-0 overflow-hidden" style={{ width: imageSize }}>
        {resolvedImage ? (
          <img
            src={resolvedImage}
            alt={recipe.name}
            className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
          />
        ) : (
          <div className="h-full w-full bg-gradient-to-br from-muted/80 to-muted/40 flex items-center justify-center">
            <ChefHat className="size-6 text-muted-foreground/20" />
          </div>
        )}
        <div className="absolute inset-0 bg-gradient-to-r from-transparent to-black/10" />

        {/* Top-right badges */}
        <div className="absolute top-1.5 right-1.5 flex flex-col items-end gap-1">
          {recipe.source === 'trader_joes' && (
            <Badge className="bg-accent-rose text-white text-[9px] h-4 px-1.5 border-0 shadow-md">
              TJ&apos;s
            </Badge>
          )}
          {isFridgeActive && fridgeScore != null && fridgeScore > 0 && (
            <FridgeMatchBadge score={fridgeScore} />
          )}
        </div>

        {/* Bottom-left: time badge */}
        {totalTime > 0 && (
          <div className="absolute bottom-1.5 left-1.5">
            <Badge className="bg-black/50 text-white backdrop-blur-sm border-0 text-[9px] h-4 px-1.5 shadow-sm">
              <Clock className="size-2 mr-0.5" />
              {totalTime}m
            </Badge>
          </div>
        )}
      </div>

      {/* Right content */}
      <div className="flex-1 min-w-0 flex flex-col justify-between p-2.5">
        <div className="space-y-1 min-w-0">
          <div className="flex items-start justify-between gap-1.5">
            <h3 className="text-sm font-semibold leading-snug line-clamp-1 flex-1 min-w-0">
              {recipe.name}
            </h3>
            <div className="flex items-center gap-1 shrink-0">
              {recipe.is_favorite && (
                <Star className="size-3.5 fill-accent-gold text-accent-gold" />
              )}
              <div
                className="opacity-0 group-hover:opacity-100 transition-opacity"
                onClick={(e) => e.stopPropagation()}
              >
                <AddToMealPlanPopover recipeId={recipe.id} recipeName={recipe.name} variant="icon" />
              </div>
            </div>
          </div>

          {recipe.description && (
            <p className="text-xs text-muted-foreground leading-snug line-clamp-2 min-w-0">
              {recipe.description}
            </p>
          )}

          {hasNutrition && <NutritionRow recipe={recipe} />}
        </div>

        {/* Bottom meta row */}
        <div className="flex items-center gap-1.5 flex-wrap min-w-0">
          {recipe.servings && (
            <span className="flex items-center gap-0.5 text-[11px] text-muted-foreground">
              <Users className="size-3" />
              {recipe.servings}
            </span>
          )}
          {recipe.difficulty && (
            <Badge
              variant="outline"
              className={cn('text-[10px] h-4 px-1.5', difficultyColors[recipe.difficulty])}
            >
              {recipe.difficulty}
            </Badge>
          )}
          {cookCount != null && cookCount > 0 && (
            <span className="flex items-center gap-0.5 text-[11px] text-accent-sage/80">
              <CalendarCheck className="size-3" />
              {cookCount}×
            </span>
          )}
          {recipe.tags?.slice(0, 2).map((tag) => (
            <Badge key={tag} variant="secondary" className="text-[10px] h-4 px-1.5 font-normal">
              {tag}
            </Badge>
          ))}
          {(recipe.tags?.length ?? 0) > 2 && (
            <span className="text-[10px] text-muted-foreground">
              +{recipe.tags!.length - 2}
            </span>
          )}
        </div>
      </div>
    </div>
  )
}

export function RecipeCardSkeleton() {
  return (
    <Card className="overflow-hidden">
      <CardContent className="p-0">
        <Skeleton className="aspect-[4/3] w-full rounded-none" />
        <div className="space-y-1 px-2.5 py-2">
          <Skeleton className="h-3.5 w-3/4" />
          <div className="flex gap-1">
            <Skeleton className="h-3.5 w-8" />
            <Skeleton className="h-3.5 w-10" />
          </div>
        </div>
      </CardContent>
    </Card>
  )
}

export function RecipeCardCompactSkeleton() {
  return (
    <div className="flex flex-row rounded-lg border border-border/40 bg-card/50 overflow-hidden h-[140px]">
      <Skeleton className="w-[140px] shrink-0 rounded-none" />
      <div className="flex-1 p-2.5 flex flex-col justify-between">
        <div className="space-y-1.5">
          <Skeleton className="h-4 w-4/5" />
          <Skeleton className="h-3 w-full" />
          <Skeleton className="h-3 w-3/4" />
        </div>
        <div className="flex gap-2">
          <Skeleton className="h-3.5 w-10" />
          <Skeleton className="h-3.5 w-12" />
          <Skeleton className="h-3.5 w-14" />
        </div>
      </div>
    </div>
  )
}
