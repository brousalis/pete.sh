...
import { WorkoutRouteMap as GpsRouteMap } from '@/components/coach/activity/workout-route-map'
...
function swimYards(workout: AppleWorkout): number | null {
  if (workout.distance_meters != null && Number(workout.distance_meters) > 0) {
    return metersToYards(Number(workout.distance_meters))
  }
  if (workout.distance_miles != null && workout.distance_miles > 0) {
    return metersToYards(workout.distance_miles * 1609.344)
  }
  return null
}

/** Stored pace is sec/100m; athlete trains SCY. */
function swimPacePer100yd(secPer100m: number): number {
  return secPer100m * 0.9144
}
    ...
    <div className={cn('space-y-3', className)}>
      ...
      {hrCadenceCorrelation && (
        <div className="rounded-lg bg-muted/20 p-2.5">
          <div className="mb-1.5 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">HR ├ù Cadence Drift</div>
...
// ============================================
// ROUTE MAP (for outdoor workouts) — uses Google Maps via GpsRouteMap
// ============================================

function OutdoorRouteSection({ route, hrSamples }: { route: RouteData; hrSamples: HrSample[] }) {
...
export function EnhancedWorkoutDetailView({ workoutId, onBack, className }: EnhancedWorkoutDetailViewProps) {
  ...
  useEffect(() => {
    async function fetchWorkout() {
      try {
        const response = await fetch(`/api/apple-health/workout/${workoutId}`, {
          credentials: 'include',
        }).then((r) => r.json()) as { success: boolean; data?: WorkoutDetailResponse }
  ...
  const isSwimming = workout.workout_type === 'swimming'
  ...
  const yards = isSwimming ? swimYards(workout) : null
  const swimLengths = (data.swimLengths ?? []).filter((length) => !length.is_rest)
    ...
    <div className={cn('space-y-3 px-1 py-2', className)}>
      ...
      <div className="rounded-xl border border-border/50 bg-card p-4">
        ...
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 border-t border-border/30 pt-3">
          ...
          {isSwimming && yards != null ? (
            <div>
              <div className="text-[10px] text-muted-foreground uppercase tracking-wide">Distance</div>
              <div className="text-base font-semibold tabular-nums text-accent-azure">{yards}<span className="ml-0.5 text-xs text-muted-foreground font-normal">yd</span></div>
            </div>
          ...
          {isSwimming && workout.swimming_lap_count != null && (
            <div>
              <div className="text-[10px] text-muted-foreground uppercase tracking-wide">Lengths</div>
              <div className="text-base font-semibold tabular-nums text-accent-teal">{workout.swimming_lap_count}</div>
            </div>
          )}
          {isSwimming &&
            ...
            <div>
              ...
              <div className="text-base font-semibold tabular-nums text-accent-sage">
                {formatSwimPace(swimPacePer100yd(workout.swimming_avg_pace_per_100))}
                <span className="ml-0.5 text-xs text-muted-foreground font-normal">/100yd</span>
              </div>
            </div>
          )}
          {isSwimming && workout.swimming_avg_swolf != null && (
            <div>
              <div className="text-[10px] text-muted-foreground uppercase tracking-wide">SWOLF</div>
              <div className="text-base font-semibold tabular-nums">{workout.swimming_avg_swolf}</div>
            </div>
          )}
          {isSwimming && workout.swimming_stroke_count != null && (
            <div>
              <div className="text-[10px] text-muted-foreground uppercase tracking-wide">Strokes</div>
              <div className="text-base font-semibold tabular-nums">{workout.swimming_stroke_count}</div>
            </div>
          )}
      ...
      {/* ============================================ */}
      {/* TIER 1b: SWIM SUMMARY */}
      {/* ============================================ */}
      {isSwimming && (yards != null || workout.swimming_lap_count != null || swimLengths.length > 0) && (
        <div className="rounded-xl border border-border/50 bg-card p-3">
          <div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            <Waves className="size-3.5 text-accent-azure" />
            Swim
            {workout.swimming_pool_length_meters != null && (
              <span className="font-normal normal-case tracking-normal">
                · {Math.round(Number(workout.swimming_pool_length_meters) * 1.09361)} yd pool
              </span>
            )}
          </div>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {yards != null && (
              <div className="rounded-lg bg-muted/40 px-2.5 py-2">
                <div className="text-[10px] text-muted-foreground uppercase tracking-wide">Yards</div>
                <div className="text-sm font-semibold tabular-nums text-accent-azure">{yards}</div>
              </div>
            )}
            {workout.swimming_lap_count != null && (
              <div className="rounded-lg bg-muted/40 px-2.5 py-2">
                <div className="text-[10px] text-muted-foreground uppercase tracking-wide">Lengths</div>
                <div className="text-sm font-semibold tabular-nums">{workout.swimming_lap_count}</div>
              </div>
            )}
            {workout.swimming_avg_pace_per_100 != null && workout.swimming_avg_pace_per_100 > 0 && (
              <div className="rounded-lg bg-muted/40 px-2.5 py-2">
                <div className="text-[10px] text-muted-foreground uppercase tracking-wide">/100 yd</div>
                <div className="text-sm font-semibold tabular-nums text-accent-sage">
                  {formatSwimPace(swimPacePer100yd(workout.swimming_avg_pace_per_100))}
                </div>
              </div>
            )}
            {workout.swimming_avg_swolf != null && (
              <div className="rounded-lg bg-muted/40 px-2.5 py-2">
                <div className="text-[10px] text-muted-foreground uppercase tracking-wide">Avg SWOLF</div>
                <div className="text-sm font-semibold tabular-nums">{workout.swimming_avg_swolf}</div>
              </div>
            )}
          </div>
          {swimLengths.length > 0 && (
            <div className="mt-3 max-h-48 overflow-y-auto">
              <div className="mb-1 grid grid-cols-[2.5rem_1fr_1fr_1fr] gap-2 text-[10px] uppercase tracking-wide text-muted-foreground">
                <span>#</span>
                <span>Time</span>
                <span>Strokes</span>
                <span>SWOLF</span>
              </div>
              {swimLengths.slice(0, 40).map((length) => (
                <div
                  key={length.length_number}
                  className="grid grid-cols-[2.5rem_1fr_1fr_1fr] gap-2 border-t border-border/30 py-1 text-xs tabular-nums"
                >
                  <span className="text-muted-foreground">{length.length_number}</span>
                  <span>{formatDuration(length.duration_seconds)}</span>
                  <span>{length.stroke_count ?? '—'}</span>
                  <span>{length.swolf ?? '—'}</span>
                </div>
              ))}
            </div>
          )}
            ...
            <div className="rounded-xl border border-border/50 bg-card p-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="rounded-lg bg-muted/20 p-2">
                  ...
                  <div className="text-[10px] text-muted-foreground mt-0.5">{analytics.cardiacDrift.firstHalfAvgHr} ΓåÆ {analytics.cardiacDrift.secondHalfAvgHr} bpm</div>
                </div>
                <div className="rounded-lg bg-muted/20 p-2">
      ...
      {hasRoute && (
        <div className="rounded-xl border border-border/50 bg-card p-3">
          <div className="grid gap-3 lg:grid-cols-2">
            <OutdoorRouteSection route={route!} hrSamples={data.hrSamples} />
