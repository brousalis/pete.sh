...
export interface AppleWorkout {
  ...
  /** Seconds per 100 meters (ingest convention) */
  swimming_avg_pace_per_100?: number | null
  // Coach enrichment (coach_activity_v / coach_activity_load)
  sport?: string | null
  tss?: number | null
...
function getWorkoutIcon(type: string, size: 'sm' | 'md' | 'lg' = 'md') {
  ...
  const icons: Record<string, React.ReactNode> = {
    ...
    swimming: <Waves className={sizeClass} />,
...
const WORKOUT_ICONS: Record<string, React.ReactNode> = {
  ...
  swimming: <Waves className="size-5" />,
...
/** Meters → yards for SCY display. */
function metersToYards(meters: number): number {
  return Math.round(meters * 1.09361)
}

/** Seconds per 100m → seconds per 100yd. */
function pacePer100mToPer100yd(secPer100m: number): number {
...
function swimDistanceYards(workout: AppleWorkout): number | null {
...
function formatWorkoutDistance(workout: AppleWorkout): string | null {
  if (workout.workout_type === 'swimming') {
    const yards = swimDistanceYards(workout)
    return yards != null ? `${yards} yd` : null
  }
  if (workout.distance_miles != null && workout.distance_miles > 0) {
    return `${workout.distance_miles.toFixed(2)} mi`
  }
  return null
}
        ...
        <div className="mt-4 rounded-lg bg-muted/30 px-3 py-2.5">
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="text-[10px] text-muted-foreground">
              {currentWeight} ΓåÆ {WEIGHT_GOAL} lbs
            </span>
          </div>
