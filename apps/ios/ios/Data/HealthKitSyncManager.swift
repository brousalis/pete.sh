...
final class HealthKitSyncManager {
    ...
    func syncHistoricalWorkouts(days: Int = 90, forceResync: Bool = false) async -> HistoricalSyncResult {
        ...
        do {
            ...
            if workouts.isEmpty {
                ...
                log("Check: Settings > Health > petehome > Workouts = ON")
            }
        } catch {
    ...
    private func buildWorkoutPayload(workout: HKWorkout) async throws -> WorkoutPayload {
        ...
        let isRunning = HealthKitPetehome.includes(workout, .running)
        ...
        if var cycling = cyclingMetricsResult {
            cycling.powerZones = HealthKitPetehome.nativeCyclingPowerZones(from: workout)
        ...
        let nativeZones = HealthKitPetehome.nativeHeartRateZones(from: workout)
        ...
        let appleHealthWorkout = AppleHealthWorkout(
            ...
            activities: {
                let legs = HealthKitPetehome.activities(from: workout)
    ...
    private func querySwimmingMetrics(for workout: HKWorkout) async -> PetehomeSwimmingMetrics? {
        guard HealthKitPetehome.includes(workout, .swimming) else { return nil }
    ...
    private func queryCyclingMetrics(for workout: HKWorkout) async throws -> PetehomeCyclingMetrics? {
        ...
        guard HealthKitPetehome.includes(workout, .cycling) else { return nil }
    ...
    private func queryRoute(for workout: HKWorkout) async throws -> PetehomeWorkoutRoute? {
        let isOutdoor = HealthKitPetehome.isOutdoorCandidate(workout)
    ...
    func queryDailyMetrics(for date: Date) async throws -> PetehomeDailyMetrics {
        ...
        async let rmssdSeries = queryHRVSeries(
            type: HealthKitPetehome.rmssdType,
            metric: "rmssd",
            windowStart: hrvWindowStart,
            windowEnd: hrvWindowEnd,
            dayStart: startOfDay,
