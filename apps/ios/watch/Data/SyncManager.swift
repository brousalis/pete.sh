...
final class SyncManager {
    ...
    func syncHistoricalWorkouts(days: Int = 90) async -> HistoricalSyncResult {
        ...
        for (index, workout) in workoutsToSync.enumerated() {
            ...
            do {
                // Try to match to a petehome day based on workout type and day of week
    ...
    /// Try to match a HealthKit workout to a petehome day based on workout type and day of week
    ...
    private func matchWorkoutToDay(_ workout: HKWorkout) -> Day? {
        ...
        // Convert to petehome day number (Mon=1, Sun=7)
        let dayNumber: Int
        if weekday == 1 {
            dayNumber = 7 // Sunday
        } else {
