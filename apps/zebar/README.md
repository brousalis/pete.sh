![Screenshot of petehome in use with GlazeWM, previewing multiple themes and rounded settings](https://github.com/user-attachments/assets/08b4be9a-e405-4217-814b-aac7acd6e0f5)
...
1.  Clone the repository to your local machine inside the `.glzr` directory (e.g., `C:/Users/<USER>/.glzr/zebar` on Windows):
    ...
    ```sh
    git clone https://github.com/mushfikurr/petehome.git
    cd petehome
    ```
...
`petehome` is highly configurable through the **config-widget**. To open it, click on the left-most button (Script Launcher), and then the "Gear Icon".
...
```sh
npm run build:watch -w @petehome/<widget-name>
```
