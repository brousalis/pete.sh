...
final class ProgressAnalytics {
    ...
    struct WeeklyVolume: Identifiable {
        let id = UUID()
        let weekStart: Date
        let totalVolume: Double // sum of (sets × reps × weight)
        let totalSets: Int
        let totalReps: Int
    ...
    func getWeeklyVolumes(weeks: Int = 8) -> [WeeklyVolume] {
        guard let modelContext = modelContext else { return [] }
        ...
        for weekOffset in 0..<weeks {
            ...
            do {
                ...
                for log in logs {
                    ...
                    totalSets += sets
                    totalReps += reps
                    workoutDays.insert(dateFormatter.string(from: log.date))
                }
                
                // Count PRs for the week
                let prPredicate = #Predicate<PersonalRecord> { pr in
                    pr.date >= mondayOfWeek && pr.date < weekEnd
                }
                let prDescriptor = FetchDescriptor<PersonalRecord>(predicate: prPredicate)
                let prCount = (try? modelContext.fetch(prDescriptor).count) ?? 0
                ...
                volumes.append(WeeklyVolume(
                    ...
                    totalSets: totalSets,
                    totalReps: totalReps,
                    workoutCount: workoutDays.count,
                    prCount: prCount
                ))
            } catch {
                continue
        ...
        return volumes.reversed() // chronological order
    }
